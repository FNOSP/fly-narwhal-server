...
public class BlackFrameAltAnalyzer {
    ...
    private static final int maximumTimeSkip = 15;
    ...
    public Segment detectCredits(QueuedEpisode episode) {
        try {
            ...
            List<BlackFrame> blackFrames = ffmpegWrapper.detectBlackFrames(
                    episode.getPath(), range, 0, config.getBlackFrameThreshold()); // amount=0 to capture all, then filter

            if (blackFrames.isEmpty()) {
                return null;
            }

            List<CreditScene> scenes = detectCreditScenes(blackFrames, config.getBlackFrameMinimumPercentage());
            if (scenes.isEmpty()) {
                return null;
            }
